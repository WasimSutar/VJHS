...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (uri.endsWith("holidays.upload")) {
			...
			getServletContext().getRequestDispatcher("/upload_holiday_list2.jsp").forward(request, response);
		} else if (uri.endsWith("academic.upload")) {
			UPLOAD_DIRECTORY = "upload_academic";
			String message = uploadFiles(request, response);
			request.setAttribute("message", message);
			getServletContext().getRequestDispatcher("/upload_academic_calender2.jsp").forward(request, response);

		} else if (uri.endsWith("schooltimings.upload")) {
			UPLOAD_DIRECTORY = "upload_schooltimings";
			String message = uploadFiles(request, response);
			request.setAttribute("message", message);
			getServletContext().getRequestDispatcher("/upload_school_timings2.jsp").forward(request, response);
