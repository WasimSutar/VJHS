...
public class Profile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (uri.endsWith("overview.profile")) {
			...
			FeeStructure feeStructure = feeStructureOperations.getFeeStructure(className);
			request.setAttribute("feeStructure", feeStructure);
			request.getRequestDispatcher("edit_fees_structure2.jsp").forward(request, response);
		} else if (uri.endsWith("editClassStrength.profile")) {
			String className = request.getParameter("className");
			Class_Strength class_Strength = classStrengthOperations.getClassStrength(className);

			request.setAttribute("classStrength", class_Strength);
			request.getRequestDispatcher("edit_class_strength.jsp").forward(request, response);
		}
			...
			if (isUpdated) {
				request.setAttribute("messageFeeStru",
			...
			request.getRequestDispatcher("feestructure2.jsp").forward(request, response);
			...
			overview = overviewOperations.getOverview();
			request.setAttribute("overview", overview);
			request.getRequestDispatcher("edit_overview2.jsp").forward(request, response);
			...
			if (isUpdate) {
				request.setAttribute("messageOverview", "Overview Updated Sucessfully");
			} else {
				request.setAttribute("messageOverview", "Overview not updated sucessfully");
			}
			overview = overviewOperations.getOverview();
			request.setAttribute("overview", overview);
			request.getRequestDispatcher("overview2.jsp").forward(request, response);
			...
			request.getRequestDispatcher("classstrength2.jsp").forward(request, response);
		}
	}
